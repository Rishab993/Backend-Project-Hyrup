...
def create_student(name: str, email: str, course: str, year: int, gpa: float, phone: str,attendance_percentage:float,address:str,emergency_contact:str, db: Session = Depends(get_db)):
    # Email check
    if "@" not in email:
    ...
    student = models.Student(name=name, email=email, course=course, year=year, gpa=gpa, phone=phone,attendance_percentage = attendance_percentage,address=address,emergency_contact=emergency_contact)
    db.add(student)
    db.commit()
